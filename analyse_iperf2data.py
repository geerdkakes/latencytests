...
#############################################################################################
# Load json output data from iperf3
...
#
# this function is tested with iperf udp
#############################################################################################
...
def merge_dataframes(df_sub, df_data):
    ...
    # print("rb_blocks: (" + str(start_s) + "," + str(end_s) + ") iperf2: (" + str(start_d) + "," + str(end_s) + ")")
    df_sub = df_sub.iloc[start_s:end_s]
    df_data = df_data.iloc[start_d:end_d]
    
    rows_sub = df_sub.shape[0]
    rows_iperf = df_data.shape[0]
    ...
    for index, row in df_data.iterrows():
        ...
        # print("iperf(" + str(index) + "/" +str(rows_iperf) + ") -> rb(" + str(index_sub) + "/" + str(rows_sub) + ")")
        if index_sub >= rows_sub:
            index_sub=index_sub -1
        for i_s, column in enumerate(columns_to_copy):
            # print("copy: " + column + " to new index: " + str(i_s))
...
for index, file in enumerate(data_files):
    ...
    averages['percentage_lost'].append(df_iperf['perc_lost'].mean())
...
fig.add_trace(go.Scatter(x=df_total['minutes'], y=df_total['down_Mbit_per_second'], name="Mbps down"),
              row=4, col=1)
if 'perc_lost' in df_total:
    fig.add_trace(go.Scatter(x=df_total['minutes'], y=df_total['perc_lost'], name="percentage lost"),
              row=5, col=1)
