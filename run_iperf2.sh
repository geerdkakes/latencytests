...
#############################################
# Script to run an iperf2 session on a remote host
#  Variables:
#   -s <session id>
#   -M <MTU size>
#   -s_ip server IP to which the iperf2 client will connect
...
######################
# config for 40MHz 4 
# 50% downlink: iperf -c ip_Addr -u -b 100000000 -R -p 5203 -l 1344 -t 120
# 80% downlink: iperf -c ip_Addr -u -b 60000000 -R -p 5203 -l 1344 -t 120 -P 2
######################
...
do
    case "$1" in
        ...
        -protocol) protocol="$2"
            ...
            shift ;;
        -port) iperf2_port="$2"
            echo "${scriptname}: iperf2_port used: ${iperf2_port}"
            shift ;;
        --) shift
            break ;;
...
######################################################################
# iperf2 test with the following parameters:
...
#  -y C                 :csv output for later processing
#  -c <ip address>
...
result=$?
if [ ${result} -eq 1 ]; then
    echo "${scriptname}:  Iperf2 session has closed down"
else
    echo "${scriptname}:  iperf2 session on server still running!!!"
    echo "${scriptname}:  closing server iperf2 session for session_id $session_id"
    kill $iperf_PID
fi
##########################################
# retrieve device logfile 
##########################################
scp ${userid_device}@${deviceIP}:${data_dir_device}/${session_id}/device_iperf2_${testdate}.csv  ${data_dir_server}/${session_id}/
echo "${scriptname}: stored iperf2 output at ${data_dir_server}/${session_id}/device_iperf2_${testdate}.csv"
