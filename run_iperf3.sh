...
echo "${scriptname}: ${iperf3_app} -s -1 -J  -p ${iperf3_port} > ${data_dir_server}/${session_id}/server_iperf3_${testdate}.json"
result=1

while [ ${result} -eq 1 ]; do
    ${iperf3_app} -s -1 -J -p ${iperf3_port} > ${data_dir_server}/${session_id}/server_iperf3_${testdate}.json &
    iperf_PID=$!
    sleep 1
    kill -0 $iperf_PID
    result=$?
...
echo "${scriptname}: ssh ${userid_device}@${deviceIP} \"${iperf3_app} -p ${iperf3_port} -c ${serverIP} -l ${DATALENGTH} -N -t ${time} -T ${session_id} -O 1 -J ${dir_var} -P ${streams} ${bitrateoption} ${udp_tcp_specific} > ${data_dir_device}/${session_id}/device_iperf3_${testdate}.json\""
ssh ${userid_device}@${deviceIP} "${iperf3_app} -c ${serverIP} -p ${iperf3_port} -l ${DATALENGTH} -N -t ${time} -T ${session_id} -O 1 -J ${dir_var} -P ${streams} ${bitrateoption} ${udp_tcp_specific} > ${data_dir_device}/${session_id}/device_iperf3_${testdate}.json"
if [ ${?} -eq 1 ]; then
